...
- **Hub**: `?page=hub` (also the default with no `?page=` at all) lists
  every tool straight from `PAGES`, as a list-on-the-left/detail-panel-
  on-the-right layout (`AppScript/Hub.html`, which — like every tool's
  page — has no `tools/` source counterpart; it's only ever existed as
  a deployed Apps Script file, so there's nothing to hand-sync for it).
  Selecting a tool shows a live, interactive preview: a genuine
  self-contained Export of a small hand-built demo project, mounted via
  `<iframe>.srcdoc`. A `PAGES` entry opts into this with an optional
  `previewFile` key (e.g. `previewFile: 'PreviewAnimatedSlidesV2'`)
  naming another Apps Script HTML file whose entire contents are that
  demo export, generated once (driving the real tool via Playwright
  against local preview, clicking Export, saving `#export-textarea`'s
  value verbatim) and committed as a static asset — not derived live
  from a real saved project, and not regenerated automatically; a tool
  with no `previewFile` just shows a "Preview not available yet"
  placeholder. `Code.gs`'s `doGet` reads a `previewFile`'s raw content
  fresh on every hub render (same "snapshot a file's content into a JS
  string" technique as Export's own `MODULE_SOURCES`) and passes it to
  `Hub.html` as each tool's `previewHtml`. **Escaping gotcha worth
  remembering**: that content is spliced into `Hub.html`'s own
  `<script>` block as JSON, and a demo export's raw HTML necessarily
  contains its own literal `</script>` tags — left unescaped, the
  browser's HTML parser closes Hub's outer `<script>` early on the
  first one it hits, before any of it ever reaches `JSON.parse`. Fixed
  by escaping every `<` character in the JSON payload to its unicode
  form before splicing it in — a one-line `.replace(/</g, '\\u003c')`
  in `Code.gs`, easy to
  drop if this is ever regenerated by hand instead of reusing the
  existing code path. Currently wired up for `animated-slides-v2` and
  `tabbed-panels`; unverified against a live Apps Script deployment
  (see "Current status"). A `PAGES` entry also carries an optional
  `icon` (a Font Awesome 6 solid class name, e.g. `'fa-images'` — Hub.html
  loads the Font Awesome CDN stylesheet itself, unlike most other shared
  assets, since it's hub-only) shown as a small badge on the tool's list
  row and detail header (falls back to a generic icon if omitted), and an
  optional `updates` array — short "what's new" bullets, newest first,
  rendered in a scrollable panel beneath the preview. Both are meant to
  be hand-maintained in `Code.gs` alongside `title`/`description`/`status`
  as a tool actually changes, same "PAGES is the one source of truth, no
  separate manifest" pattern as everything else here — nothing
  auto-generates `updates` from commit history or CLAUDE.md. The preview
  "window" itself is always a strict 4:3 box (`fitViewportFrame()` in
  Hub.html, a letterbox-fit against whatever space the detail panel
  actually has, recalculated on resize) — independent of whatever aspect
  ratio a given tool's own export canvas happens to be, so a 16:9 canvas
  (v2) letterboxes inside it rather than dictating the window's shape.
  **Ordering trap worth knowing if this is touched again**: the update-notes
  panel and the preview frame are flex siblings inside `.hub-detail`, and
  the notes panel's real height depends on its content — `renderDetail()`
  therefore calls `renderUpdates()` *before* `fitViewportFrame()`. Doing
  it the other way once actually shipped visibly broken: the frame got
  measured while the (still-empty) notes panel had zero height, so the
  computed viewport came out too large and visibly overlapped the header
  above it the instant the notes content was added afterward.
...
- **Hub preview panel**: new — list+detail layout with a live
  ...
  `previewHtml`/`toolsJson` payloads substituted for Apps Script's
  scriptlets, Playwright driving the actual page: tool switching swaps
  the iframe's `srcdoc`, the no-`previewFile` placeholder path works,
  and — with GSAP/fonts reachable — the embedded demo exports render
  and are genuinely interactive). **Not yet verified against a live
  Apps Script deployment** — `Code.gs` and `Hub.html` need pasting in
  and redeploying, then a real render checked in a browser, before this
