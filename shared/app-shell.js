...
const Shell = (() => {
   ...
   *
   *  Folder browsing (all optional, opt-in — omit all of these and this
   *  renders exactly as it did before folders existed):
   *    folders: [{ name }]        subfolders at the current level
   *    currentFolder: string      '/'-separated path of the level being shown ('' = root)
   *    onOpenFolder(path)         fires when a folder row (or breadcrumb segment) is clicked
   *    onNewFolder(currentFolder) fires when "New folder" is clicked
   *    onDeleteFolder(path)       fires (after an in-dialog confirm) when a folder's delete icon is clicked
   */
  function renderProjectList(containerEl, projects, onSelect, actions = {}) {
    containerEl.innerHTML = '';

    const { folders, currentFolder = '', onOpenFolder, onNewFolder, onDeleteFolder } = actions;
    const folderBrowsingEnabled = !!onOpenFolder;

    if (folderBrowsingEnabled) {
      const toolbar = document.createElement('div');
      toolbar.className = 'project-folder-toolbar';

      const crumb = document.createElement('div');
      crumb.className = 'project-breadcrumb';
      const rootLink = document.createElement('span');
      rootLink.className = 'breadcrumb-link';
      rootLink.textContent = 'All projects';
      rootLink.addEventListener('click', () => onOpenFolder(''));
      crumb.appendChild(rootLink);
      let pathSoFar = '';
      (currentFolder ? currentFolder.split('/') : []).forEach((seg) => {
        ...
        link.addEventListener('click', () => onOpenFolder(target));
      ...
      if (onNewFolder) {
        ...
        newFolderBtn.addEventListener('click', () => onNewFolder(currentFolder));
    ...
    if ((!projects || projects.length === 0) && (!folders || folders.length === 0)) {
      const empty = document.createElement('div');
      empty.className = 'project-empty';
      empty.textContent = 'No projects yet — save one to see it here.';
    ...
    (folders || []).forEach(f => {
      ...
      row.className = 'project-row project-row-folder';
      const path = currentFolder ? `${currentFolder}/${f.name}` : f.name;

      const nameSpan = document.createElement('span');
      nameSpan.className = 'project-name';
      nameSpan.innerHTML = `<i class="fa-solid fa-folder"></i> ${escapeHtml(f.name)}`;
      ...
      if (onDeleteFolder) {
        ...
        deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          const ok = await confirmDialog(`Delete folder "${f.name}" and everything in it? This can't be undone.`, { heading: 'Delete folder', okLabel: 'Delete', danger: true });
          if (ok) onDeleteFolder(path);
        });
