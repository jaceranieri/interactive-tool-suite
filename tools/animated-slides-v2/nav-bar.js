...
function defaultNavStyle() {
  return {
    ...
    buttonBackgroundEnabled: true, buttonInactiveBackgroundEnabled: true,
...
function styleNavButton(btn, isActive, navStyle) {
  // Background/border-bottom can be switched off per state (active vs
  // inactive) — defaults to true so existing projects render identically
  // to before this was added. Border-bottom stays present but transparent
  // rather than removed outright, so turning the background off doesn't
  // shift the button's height/layout.
  const bgEnabled = isActive
    ? navStyle.buttonBackgroundEnabled !== false
    : navStyle.buttonInactiveBackgroundEnabled !== false;
  ...
  btn.style.borderBottom = '4px solid ' + (bgEnabled ? (isActive ? navStyle.buttonBottomColor : navStyle.buttonInactiveBottomColor) : 'transparent');
  btn.style.background = bgEnabled ? (isActive ? navStyle.buttonColor : navStyle.buttonInactiveColor) : 'transparent';
