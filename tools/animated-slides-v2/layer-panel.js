...
function renderLayerPanel(container, slideManager, canvasEditor) {
  ...
  topToBottom.forEach((id) => {
    ...
    row.addEventListener('click', (e) => {
      // origin: 'layers-panel' — see CanvasEditor.select()'s doc comment.
      // Selecting/deselecting from a row should keep the Layers panel
      // open rather than auto-switching to Properties the way a real
      // canvas click does.
      const additive = e.shiftKey || e.metaKey || e.ctrlKey;
      if (additive && canvasEditor.selectedIds.has(id)) {
        canvasEditor.selectedIds.delete(id);
        canvasEditor._renderHandles();
        canvasEditor.onSelect([...canvasEditor.selectedIds], { origin: 'layers-panel' });
        renderLayerPanel(container, slideManager, canvasEditor);
        return;
      }
      canvasEditor.select(id, { additive, origin: 'layers-panel' });
      renderLayerPanel(container, slideManager, canvasEditor);
    });
